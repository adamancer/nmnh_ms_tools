...
class GeoNamesFeatures:
    ...
    def get_json(self, geoname_id):
        ...
        row = session.query(self.features) \
    ...
    def get_many(self, geoname_ids):
        ...
        session = self.session()
        rows = session.query(self.features) \
    ...
    def search_json(self, st_name, limit=100, **kwargs):
        ...
        query = session.query(self.features) \
                       .join(self.names) \
                       .filter(*fltr) \
                       .order_by(sort_order) \
                       .limit(limit)
        ...
        if len(results) < limit and len(st_name) >= 3:
            fltr[0] = or_(self.names.st_name.like(st_name + '%'),
        ...
        if source is None:
            ...
            if loc_ids:
                session.query(self.names) \
                       .filter(self.names.geoname_id.in_(loc_ids)) \
    ...
    def finalize_names(self, rec):
        if re.search(r'\bADM\dH?\b', str(rec)):
            ...
            try:
                ...
                variants = []
            new = {n.title().replace('-', ' ')
                   for n in variants if n not in names}
            if new:
    ...
    def remove_unwanted_names(self):
        ...
        for geoname_id, st_name in self.unwanted_names.items():
            for row in session.query(self.features) \
                              .filter(self.features.geoname_id == geoname_id):
                synonyms = []
    ...
    def fill_record(self, row, session):
        ...
        if not row.ocean and result.get('fcl') == 'U':
            ...
            try:
                ...
                session.query(self.names) \
                       .filter(self.names.geoname_id == geoname_id) \
                       .update({self.names.ocean: ocean},
                               synchronize_session=False)
            except KeyError:
                pass
        session.commit()
        return session.query(self.features) \
                      .filter_by(geoname_id=geoname_id) \
                      .first()
    ...
    def assign_short_country_name(self, rec):
        ...
        if rec.get('fcl') == 'A' and str(rec.get('fcode')) in fcodes:
            try:
                ...
                if (name != rec['name']
                    and (name.lower() in rec['name'].lower()
                         or rec['fcode'][0].startswith('P'))):
                    rec['name'] = self.get_country_name(rec['country_code'])
            except (AttributeError, KeyError):
                logger.warning("Country code '{}' not found".format(rec['country_code']))
        return rec
