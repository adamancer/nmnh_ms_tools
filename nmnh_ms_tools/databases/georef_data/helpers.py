...
def get_alt_geometry(geoname_id):
    ...
    session = Session()
    row = session.query(AlternativePolygons.geometry,
                        AlternativePolygons.source) \
...
def get_preferred(
    name, country_code=None, admin_code_1=None, admin_code_2=None
):
    ...
    # Run query
    session = Session()
    result = session.query(PreferredLocalities.geonames_id) \
                    .filter(*query) \
                    .first()
    session.close()
    return result
...
def composite_natural_earth():
    ...
    for comp_name, names in composites.items():
        geoms = []
        for row in session.query(GeoNamesToNaturalEarth) \
                          .filter(GeoNamesToNaturalEarth.name.in_(names)):
