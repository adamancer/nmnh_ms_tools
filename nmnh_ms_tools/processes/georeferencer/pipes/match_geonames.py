...
CODES_TO_FEATURE = {'-'.join(sorted(set(v))): k
                    for k, v in FEATURE_TO_CODES.items()}
...
class MatchGeoNames(MatchPipe):
    ...
    def __init__(self, *args, username=None, use_local=None,
                 include_feature_classes=False, **kwargs):
    ...
    def _georeference(self, feature, codes=None, size='normal'):
        ...
        if (self.field is not None
            and self.field.startswith(('country', 'state_province', 'county'))):
    ...
    def _georeference_actual(self, name, st_name=None, codes=None,
                             size='normal', std_func=None, use_cache=None):
        ...
        if not records and size not in {'small', 'very large'}:
            ...
            records = self._georeference_actual(name, st_name, codes, 'very large', use_cache=False)
        # Log result
        if len(codes) > 20:
            codes = codes[:20] + ['...']
    ...
    def filter_records(self, records, name, codes=None, min_size=0, std_func=None):
        ...
        for i, site in enumerate(records):
            ...
            try:
                ...
                if (
                    site.country and not site.country_code
                    or site.state_province and not site.admin_code_1
                    or site.county and not site.admin_code_2
                ):
                    raise
