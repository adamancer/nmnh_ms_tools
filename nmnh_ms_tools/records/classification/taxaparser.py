...
class TaxaParser:
    ...
    config = yaml.safe_load(open(os.path.join(CONFIG_DIR, 'config_classification.yml'), 'r'))
    ...
    def _reconcile_compound_names(startswith, endswith):
        ...
        while (startswith
               and endswith
               and startswith[-1].end > endswith[-1].start):
            if startswith[-1].taxon.is_official():
                ...
                raise ValueError(mask.format([s.phrase for s in startswith],
                                             [s.phrase for s in endswith]))
