...
class Taxon(BaseDict):
    ...
    def _parse_emu(self, rec):
        ...
        # Map simple EMu records using the tree
        if not set(rec) - {"irn", "ClaScientificName"}:
            self._parse_name(rec.get("irn", rec["ClaScientificName"]))
            return
    ...
    def _parse_name(self, name):
        ...
        try:
            ...
            # Set defaults for an unrecognized taxon if tree or taxon not found
    ...
    def classify(self, preferred=True):
        ...
        if "Sedimentary rock" in parents:
