...
class DirectionParser(Parser):
    ...
    def parse(self, text):
        ...
        mod1 = r'(?:[a-z\-\']+\.?(?: [a-z\-\']+)* )?'
        mod2 = (r'(?: or so| \(?(?:air(?:line)?|map|naut(?:ical)?|road|trail)'
                r'(?: distance)?\)?)?')
        mod3 = (r'(?:due |\(?by [a-z\-\']+\)? |(?:up|down)stream from '
        ...
        if match is not None:
            ...
            if any(groups[0]):
                ...
                if (not (self.min_dist or self.max_dist)
                    and not re.search(pattern, self.verbatim, flags=re.I)):
        ...
        if (bool(self.min_dist) == bool(self.unit)
            and self.bearing
            and self.feature):
