...
class ModifiedParser(Parser):
    ...
    def _extract_vicinity_string(self, val):
        ...
        pattern = (r'(^{0}( of)?\b|\({0}\);'
                   r'|\b(and )?(area|surroundings|vicinity))').format(terms)
        try:
...
def is_adjective(val):
    ...
    mask = (r'\b((north|south|east|west)ern(most)?|central)\b'
            r'(?! ({})( of\b|$))')
...
def is_long(val):
    ...
    mask = (r'\b((north|south|east|west){{1,2}}(ern(most)?)?|central)\b'
            r'(?! ({})( of\b|$))')
    pattern = mask.format('|'.join(OF_WORDS))
...
def get_modified_patterns(match_start=False, match_end=False, masks=None):
    ...
    mask = r'{}{{}}{}'.format('^' if match_start else r'\b',
                              '$' if match_start else r'\b')
