...
class Cluster:
    ...
    def cluster(self, val, minlen=4, maxlen=6, related=None):
        ...
        if (
            ...
            if self.all_valid_catnums(re.split(r" +", val)) or self.all_valid_catnums(
                re.split(r"[- ]+", val)
            ):
    ...
    def _validate_last(self, vals):
        ...
        if (despaced.isdigit() and len(despaced) <= 6) or self.ends_with_range(val):
            val = val.replace(" ", "")
