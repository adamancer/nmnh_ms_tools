...
class Standardizer:
    ...
    def get_params(self, kwargs):
        for key in kwargs:
            ...
            if kind is not None:
                ...
                if (key == 'replace_endings'
                    and isinstance(val, (list, set, tuple))):
                        val = {s: '' for s in val}
                        kwargs[key] = val
    ...
    def _std(self, val, pre=None, post=None, **kwargs):
        ...
        for i, word in enumerate(re.findall(pattern, val.replace('_', '-'))):
            ...
            if (not word
                or not word.isnumeric() and len(word) < params['minlen']
                or word in params['stopwords']
                or not i and word in params['strip_first']
                or i == (len(words) - 1) and word in params['strip_last']):
                    continue
            words.append(word)
